...
@dataclass
# Copied from transformers.models.idefics.modeling_idefics.IdeficsCausalLMOutputWithPast with Idefics->MplugOwl
class MplugOwlCausalLMOutputWithPast(ModelOutput):
    """
...
class MplugOwlAbstractorMLP(nn.Module):
    def __init__(self, config: MplugOwlAbstractorConfig):
        super().__init__()
        self.config = config
...
class MplugOwlAbstractorMultiHeadAttention(nn.Module):
    def __init__(self, config: MplugOwlAbstractorConfig):
        super().__init__()
        self.config = config
...
class MplugOwlAbstractorCrossOutput(nn.Module):
    def __init__(self, config: MplugOwlAbstractorConfig):
        super().__init__()
        dim = config.hidden_size
        self.out_proj = nn.Linear(dim, dim, bias=True)
        self.norm2 = LayerNormFp32(dim)
        self.mlp = MplugOwlAbstractorMLP(config)
...
class MplugOwlAbstractorAttention(nn.Module):
    def __init__(self, config: MplugOwlAbstractorConfig):
        super().__init__()
        self.attention = MplugOwlAbstractorMultiHeadAttention(config)
        self.output = MplugOwlAbstractorCrossOutput(config)
...
class MplugOwlAbstractorLayer(nn.Module):
    def __init__(self, config: MplugOwlAbstractorConfig, layer_idx: int):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.layer_idx = layer_idx

        self.crossattention = MplugOwlAbstractorAttention(config)
...
class MplugOwlAbstractorEncoder(nn.Module):
    def __init__(self, config: MplugOwlAbstractorConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList(
            [MplugOwlAbstractorLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MplugOwPreTrainedModel(PreTrainedModel):
...
class MplugOwlAbstractorModel(MplugOwPreTrainedModel):
    def __init__(self, config: MplugOwlConfig) -> None:
        ...
        self.encoder = MplugOwlAbstractorEncoder(config)
...
class MplugOwlForCausalLM(MplugOwPreTrainedModel):
    def __init__(
        self,
        ...
        if abstractor is None:
            abstractor = MplugOwlAbstractorModel(config.abstractor_config)
    ...
    def forward(
    ...
    ) -> Union[Tuple, MplugOwlCausalLMOutputWithPast]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        return MplugOwlCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
        ...
        model_inputs.update(
            {
                ...
                "patch_position_mask": patch_position_mask,
            }
        )
        return model_inputs
